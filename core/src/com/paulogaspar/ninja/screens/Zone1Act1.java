...
public class Zone1Act1 implements Screen{
	...
	public void init(){
		...
		String message[] = {"Welcome student","You have proved yourself so far...",
				...
				"I hope so","Your training begins once you...","Go Through that door","Go now"};
	...
	private void update(float delta){
		...
		if(!options && !volume){
			...
			if(Gdx.input.isKeyJustPressed(Input.Keys.SPACE)&&!next_stage&&player.rect().overlaps(new Rectangle(830,130,300,200))){
				can_control = false;
				next_stage = true;
			}
			
			if(next_stage){
				main_theme.setVolume(1-stage_transition_alpha);
				stage_transition_alpha += delta*0.5f;
				if(stage_transition_alpha > 1){
					stage_transition_alpha = 1;
					main_theme.stop();
					game.setScreen(new Stage_test(game, master_volume));
					dispose();
					changed_screen = true;
					return;
				}
